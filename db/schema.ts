...
export const publicFolders = sqliteTable("public_folders", {
  ...
  tagsJson: text("tags_json").notNull().default("[]"), createdAt: text("created_at").notNull().default(sql`CURRENT_TIMESTAMP`), updatedAt: text("updated_at").notNull().default(sql`CURRENT_TIMESTAMP`),
});

export const publicFolderItems = sqliteTable("public_folder_items", {
  folderId: text("folder_id").notNull(), contributionId: text("contribution_id").notNull(), position: integer("position").notNull().default(0), createdAt: text("created_at").notNull().default(sql`CURRENT_TIMESTAMP`),
});

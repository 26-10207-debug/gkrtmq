...
const folder = await (await request('/api/folders',json({title:'Local performance folder '+unique,subject:'수학',contributionIds:[id],tags:['검증']},owner),201)).json();
const summaries = await (await request('/api/folders?mine=1&summary=1',{headers:{Cookie:owner}})).json();
const summaryFolder = summaries.folders.find(item=>item.id===folder.folder.id);
assert.equal(summaryFolder.itemCount,1); assert.equal(summaryFolder.items,undefined); assert.deepEqual(summaryFolder.tags,['검증']);
const detailFolder = await (await request('/api/folders?id='+folder.folder.id,{headers:{Cookie:owner}})).json();
assert.equal(detailFolder.folder.items[0].id,id);
const publicFolders = await (await request('/api/folders?summary=1')).json();
assert.ok(!publicFolders.folders.some(item=>item.id===folder.folder.id));
await request('/api/folders?mine=1&summary=1',{},401);

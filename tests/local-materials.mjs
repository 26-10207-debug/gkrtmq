...
const rpc = async (name,args) => (await request('/mcp', json({ jsonrpc: '2.0', id: checks, method: 'tools/call', params: { name, arguments: args } }))).json();
const init = await (await request('/mcp', json({ jsonrpc: '2.0', id: 1, method: 'initialize', params: { protocolVersion: '2025-06-18', capabilities: {}, clientInfo: { name: 'local-contract', version: '1' } } }))).json();
assert.equal(init.result.protocolVersion, '2025-06-18');
await request('/mcp', json({ jsonrpc: '2.0', method: 'notifications/initialized' }), 202);

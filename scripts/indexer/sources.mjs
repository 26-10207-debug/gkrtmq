...
export async function crawlSources({api,stateDir,stop}){
 ...
 for(const record of sources){if(stop())break;const source=JSON.parse(record.config_json);if(!record.enabled||progress[source.id]?.day===day&&progress[source.id].done)continue;const previous=progress[source.id]?.day===day?progress[source.id]:{day,count:0,seen:[]};const queue=Array.isArray(previous.pending)?previous.pending:[...source.seeds],seen=new Set(previous.seen),rules=new Map();let count=previous.count;
  while(queue.length&&count<100&&!stop()){const value=queue.shift();if(seen.has(value))continue;let url;try{url=allowed(value,source)}catch{continue;}seen.add(value);count++;
   try{if(!rules.has(url.origin)){const r=await request(url.origin+'/robots.txt',source,{robots:true});const txt=r.status===404?'':await bodyText(r,300000);rules.set(url.origin,robotsParser(url.origin+'/robots.txt',txt));}if(rules.get(url.origin).isAllowed(url.href,agent)===false)throw new Error('robots 규칙으로 본문 수집 제외');

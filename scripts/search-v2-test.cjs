...
(async()=>{
 ...
 const times=[];const start=performance.now();sqlite.exec('BEGIN');for(let i=0;i<10000;i++)add('scale'+i,'시험 색인 '+i,'한국어 국어 관성 기본 내용 '+i,{duplicate:'sha256:'+String(i).padStart(64,'0')});sqlite.exec('COMMIT');const indexMs=performance.now()-start;

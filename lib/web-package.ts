...
export type WebPackage = { files: Record<string, Uint8Array>; entry: string | null };
...
export function unpackWebPackage(bytes: Uint8Array, name: string): WebPackage {
  ...
  if (manifestName) {
    ...
    const manifest = JSON.parse(strFromU8(files[manifestName])) as { version?: number; entry?: string };

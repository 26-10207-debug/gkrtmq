...
export async function searchLibrary(DB: D1Database, input: SearchInput, diagnostics=false): Promise<SearchResponse> {
  ...
  const queries = [DB.prepare(query.sql).bind(...query.bindings), DB.prepare(`SELECT s.subject FROM (SELECT DISTINCT subject FROM search_v2_documents) s WHERE EXISTS(SELECT 1 FROM search_v2_documents d WHERE d.subject=s.subject AND ${visibleSearchDocument}) ORDER BY s.subject LIMIT 60`),DB.prepare("SELECT value FROM search_v2_control WHERE key='version'")];

import {readSearchCache,writeSearchCache,subjectQuery} from "./search-cache";
...
function termBranch(term: QueryTerm, termNo: number, variant: string, expanded: boolean, bindings: Array<string | number>) {
  ...
  return `SELECT c.document_id, ${termNo} AS term_no, ${expanded?1:0} AS expanded, c.id+CASE WHEN c.field='meta' THEN 1000000000000 ELSE 0 END AS chunk_id, CASE WHEN c.field='meta' THEN 4 ELSE 1 END AS weight FROM ${short ? "search_v2_short" : "search_v2_fts"} JOIN search_v2_chunks c ON c.id=${short ? "search_v2_short" : "search_v2_fts"}.rowid WHERE ${short ? "search_v2_short" : "search_v2_fts"} MATCH ? AND c.field<>'staging' AND instr(c.${column},?)>0`;
}
...
export function buildSearchQuery(input: SearchInput, offset = 0) {
  ...
  for (const term of excluded) { const needle = term.phrase ? term.value : compactSearch(term.value); conditions.push(`NOT EXISTS (SELECT 1 FROM search_v2_chunks x WHERE x.document_id=d.id AND x.field<>'staging' AND instr(x.${term.phrase ? "normalized" : "compact"},?)>0)`); bindings.push(needle); }
  ...
  const termWeights=Array.from({length:termCount},(_,i)=>`MAX(CASE WHEN term_no=${i} THEN weight ELSE 0 END)`);
  const weight=termWeights.join("+");
  const having=positive.length ? ` HAVING ${termWeights.map(expr=>`${expr}>0`).join(" AND ")}` : "";
  const sql = `WITH hits AS (${hits}), matched AS (SELECT document_id,${penalty} AS expansion_penalty,${weight} AS hit_weight,MIN(chunk_id) AS chunk_id FROM hits GROUP BY document_id${having}), filtered AS (SELECT d.id,d.compact_title,d.duplicate_key,d.created_at,d.view_count,m.expansion_penalty,m.hit_weight,m.chunk_id%1000000000000 AS chunk_id FROM matched m JOIN search_v2_documents d ON d.id=m.document_id WHERE ${conditions.join(" AND ")}), ranked AS (SELECT d.*,CASE WHEN ?<>'' AND compact_title=? THEN 0 WHEN ?<>'' AND instr(compact_title,?)=1 THEN 1 WHEN ${eachTitle} THEN 2 WHEN expansion_penalty=0 AND hit_weight>=4 THEN 3 WHEN expansion_penalty=0 THEN 4 ELSE 5 END AS relevance_tier FROM filtered d), duplicated_keys AS MATERIALIZED (SELECT duplicate_key FROM search_v2_documents GROUP BY duplicate_key HAVING COUNT(*)>1), grouped AS (SELECT *,COUNT(*) OVER(PARTITION BY duplicate_key) AS duplicate_count,ROW_NUMBER() OVER(PARTITION BY duplicate_key ORDER BY ${order}) AS duplicate_order FROM ranked WHERE duplicate_key IN (SELECT duplicate_key FROM duplicated_keys)), representatives AS (SELECT *,1 AS duplicate_count,1 AS duplicate_order FROM ranked WHERE duplicate_key NOT IN (SELECT duplicate_key FROM duplicated_keys) UNION ALL SELECT * FROM grouped ${input.duplicates ? "" : "WHERE duplicate_order=1"}) SELECT d.*,page.duplicate_count,page.total,page.chunk_id,c.content AS matched_content,c.location_json AS matched_location FROM (SELECT *,(SELECT COUNT(*) FROM representatives) AS total FROM representatives ORDER BY ${order} LIMIT ? OFFSET ?) page JOIN search_v2_documents d ON d.id=page.id LEFT JOIN search_v2_chunks c ON c.id=page.chunk_id`;
...
export async function searchLibrary(DB: D1Database, input: SearchInput, diagnostics=false): Promise<SearchResponse> {
  ...
  const cacheKey=signature+"|"+(input.cursor||"");const cached=await readSearchCache(DB,cacheKey,diagnostics);if(cached.hit)return cached.hit;
  const queries = [DB.prepare(query.sql).bind(...query.bindings), DB.prepare(subjectQuery),DB.prepare("SELECT value FROM search_v2_control WHERE key='version'")];
  ...
  const response:SearchResponse = { ...(diagnostics?{diagnostics:{statements:3+cached.validation.statements,rowsRead:cached.validation.rowsRead+[found,subjects,versionRows].reduce((n,r)=>n+(r.meta?.rows_read||0),0),cacheHit:false}}:{}), results, related: [...new Set(query.parsed.terms.flatMap(t => aliasesFor(t)).filter(t => !query.parsed.terms.some(x => x.value===t)))].slice(0,6), subjects: subjects.results.map(r=>String(r.subject)), nextCursor: all.length>query.limit ? encodeCursor(offset+query.limit,version,signature) : null, total: rows[0]?.total || 0, indexVersion: version, appliedFilters: query.parsed.filters, engine: "v2" };
  writeSearchCache(DB,cacheKey,response,rows as unknown as Array<Record<string,unknown>>);return response;
}
...
export async function suggestSearch(DB: D1Database, raw: string) {
  ...
  for(const group of synonymGroups) for(const value of group) if(suggestions.length<8 && (compactSearch(value).startsWith(compact)||isInitialQuery(q)&&initials(value).startsWith(q)) && value!==q && !suggestions.some(s=>s.text===value)) suggestions.push({text:value,kind:"term"});

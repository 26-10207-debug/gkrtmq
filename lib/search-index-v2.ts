...
export function deleteSectionChunks(DB:D1Database,id:string,section:TextSection,field="body"){
  const where="document_id=? AND field=? AND COALESCE(json_extract(location_json,'$.attachment'),0)=? AND COALESCE(json_extract(location_json,'$.page'),0)=? AND COALESCE(json_extract(location_json,'$.path'),'')=?";
  const values=[id,field,section.attachment??0,section.page??0,section.path??''];
  return [DB.prepare(`DELETE FROM search_v2_fts WHERE rowid IN(SELECT id FROM search_v2_chunks WHERE ${where})`).bind(...values),DB.prepare(`DELETE FROM search_v2_short WHERE rowid IN(SELECT id FROM search_v2_chunks WHERE ${where})`).bind(...values),DB.prepare(`DELETE FROM search_v2_chunks WHERE ${where}`).bind(...values)];
}
export function promoteStagedChunks(DB:D1Database,id:string){
  // An unreadable replacement must not erase text retained from an earlier extraction.
  const where="document_id=? AND field='body' AND EXISTS(SELECT 1 FROM search_v2_chunks s WHERE s.document_id=? AND s.field='staging' AND s.normalized<>'')";
  return [DB.prepare(`DELETE FROM search_v2_fts WHERE rowid IN(SELECT id FROM search_v2_chunks WHERE ${where})`).bind(id,id),DB.prepare(`DELETE FROM search_v2_short WHERE rowid IN(SELECT id FROM search_v2_chunks WHERE ${where})`).bind(id,id),DB.prepare(`DELETE FROM search_v2_chunks WHERE ${where}`).bind(id,id),DB.prepare("UPDATE search_v2_chunks SET field='body',chunk_key='body:'||substr(chunk_key,9) WHERE document_id=? AND field='staging'").bind(id)];
}

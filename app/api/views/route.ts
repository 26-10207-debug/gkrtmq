...
export async function POST(request:Request){
 ...
 await DB.batch([DB.prepare("DELETE FROM search_v2_views WHERE expires_at<?").bind(Date.now()),DB.prepare("UPDATE contributions SET view_count=view_count+1 WHERE id=? AND NOT EXISTS(SELECT 1 FROM search_v2_views WHERE id=?)").bind(id,key),DB.prepare("UPDATE search_v2_documents SET view_count=(SELECT view_count FROM contributions WHERE id=?) WHERE source_type='contribution' AND source_id=?").bind(id,id),DB.prepare("INSERT INTO search_v2_control(key,value) SELECT 'version',? WHERE NOT EXISTS(SELECT 1 FROM search_v2_views WHERE id=?) ON CONFLICT(key) DO UPDATE SET value=excluded.value").bind(crypto.randomUUID(),key),DB.prepare("INSERT OR IGNORE INTO search_v2_views(id,expires_at) VALUES(?,?)").bind(key,Date.now()+86400000)]);

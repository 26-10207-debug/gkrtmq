...
export async function POST(request:Request){
  ...
  const attachments=parsed<StoredAttachment[]>(row.attachmentsJson,[]).filter((item)=>item.role!=="corrected"),texts=parsed<string[]>(row.extractedTextsJson,[]); const attachment=attachments[index]; if(!attachment)return Response.json({error:"파일을 찾지 못했습니다."},{status:404});

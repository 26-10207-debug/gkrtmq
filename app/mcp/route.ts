...
export async function POST(request: Request) {
  ...
  const response = await handleRpc(message, (name, args) => name === "search" ? searchPublic(String(args.query), origin) : name === "fetch" ? fetchPublic(String(args.id), origin) : readPublicFile(String(args.id), Number(args.attachment || 0), args.path as string | undefined, Number(args.offset || 0), origin));

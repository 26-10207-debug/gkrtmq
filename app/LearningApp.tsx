...
import { ConceptModel, CustomMaterials, SourceSelection, conceptShapeDefinitions, emptyCustomMaterials, hasCustomMaterials, hasImageSelection } from "@/lib/custom-materials";
  ...
  if (props.asset.isUpload) {
    return (
      <main className="detail-main">
        ...
        {props.asset.customMaterials && hasCustomMaterials(props.asset.customMaterials) && <CustomMaterialsPanelV2 materials={props.asset.customMaterials} imageUrl={props.asset.fileUrl} />}
...
function ContributionScreenV2({ onBack, onPublished }: { onBack: () => void; onPublished: (item: ContributionRecord) => void }) {
  const [file, setFile] = useState<File | null>(null); const [title, setTitle] = useState(""); const [sourceNote, setSourceNote] = useState("");
  ...
  const [customMaterials, setCustomMaterials] = useState<CustomMaterials>(() => emptyCustomMaterials()); const [materialsOpen, setMaterialsOpen] = useState(false); const [extractedText, setExtractedText] = useState(""); const [extracting, setExtracting] = useState(false); const [extractMessage, setExtractMessage] = useState("");
  ...
  async function chooseFile(next: File | null) {
    setFile(next); setExtractedText(""); setExtractMessage(""); if (!next) return;
    if (isImageFile(next) || /\.pptx$/i.test(next.name)) { setExtractMessage(isImageFile(next) ? "사진은 OCR을 선택하면 텍스트로 바꿉니다." : "PPTX는 현재 기존 OCR 처리 흐름을 사용합니다."); return; }
    setExtracting(true); try { const text = await extractDocumentText(next); setExtractedText(text); setExtractMessage(text ? `본문 ${text.length.toLocaleString("ko-KR")}자를 바로 읽었습니다.` : "읽을 수 있는 본문이 없어 OCR을 선택해 보세요."); } catch { setExtractMessage("본문을 바로 읽지 못했습니다. 스캔 문서라면 OCR을 선택해 주세요."); } finally { setExtracting(false); }
  }
  async function submit(event: FormEvent) {
    event.preventDefault(); if (!file) return; setSubmitting(true); setResult(null);
    const body = new FormData(); body.set("file", file); body.set("title", title); body.set("sourceNote", sourceNote); body.set("publishMode", publishMode); body.set("ocr", String(ocr)); body.set("textOnly", String(textOnly)); body.set("splitQuestions", String(splitQuestionSet)); body.set("createRecall", String(createRecall)); body.set("customMaterials", JSON.stringify(customMaterials)); body.set("extractedText", extractedText); body.set("licenseConfirmed", String(licenseConfirmed));
  ...
  return <main className="contribution-main"><button className="back-button" type="button" onClick={onBack}>← 검색으로 돌아가기</button><div className="contribution-grid"><section className="contribution-intro"><p className="eyebrow">자료 기여</p><h1>원문에서 학습 도구까지 만드세요.</h1><p>문서의 글자를 바로 읽고, 암기·회상·예시 도구를 기여 자료와 함께 공개합니다.</p><ol><li><span>1</span><div><strong>원문 바로 읽기</strong><p>Word와 텍스트 PDF는 OCR 없이 본문을 추출합니다.</p></div></li><li><span>2</span><div><strong>자료 구체화</strong><p>원문에서 필요한 부분을 골라 암기와 회상 도구로 바꿉니다.</p></div></li><li><span>3</span><div><strong>공개·검수</strong><p>AI 검수에는 원본 대신 추출 텍스트만 전달됩니다.</p></div></li></ol></section><form className="contribution-form" onSubmit={submit}>
    ...
    <label className="upload-field"><span>파일</span><input type="file" accept=".pdf,.png,.jpg,.jpeg,.webp,.txt,.md,.docx,.pptx" onChange={(event) => void chooseFile(event.target.files?.[0] ?? null)} required /><div><strong>{file ? file.name : "파일을 선택하세요"}</strong><small>{file ? `${(file.size / 1024 / 1024).toFixed(2)}MB` : "PDF, Word, 이미지, 텍스트 · 최대 8MB"}</small></div></label>
    <label className="camera-field"><span>또는 사진 촬영</span><input type="file" accept="image/png,image/jpeg,image/webp" capture="environment" onChange={(event) => void chooseFile(event.target.files?.[0] ?? null)} /><small>휴대폰에서는 카메라 권한을 요청해 바로 촬영할 수 있습니다.</small></label>
    {file && <p className={extractMessage.includes("못") ? "document-read-status warning" : "document-read-status"}>{extracting ? "본문을 읽고 있어요…" : extractMessage || "파일을 분석할 준비가 되었습니다."}</p>}
    <fieldset className="mechanical-tools"><legend>기계적 처리 도구 <small>AI 검수와 별도로 작동</small></legend><p>{extractedText ? "문서 본문을 바로 읽었습니다. 필요하면 문제·회상 도구를 추가할 수 있고, AI 검수에는 이 텍스트만 전달됩니다." : "이미지와 스캔 문서는 OCR로 글자를 추출합니다. AI 검수에는 원본 파일이 아닌 텍스트만 전달됩니다."}</p><label><input type="checkbox" checked={ocr || (publishMode === "ai_review" && !extractedText)} disabled={publishMode === "ai_review" && !extractedText} onChange={(event) => setOcr(event.target.checked)} /> 이미지·스캔 문서 글자 인식(Azure OCR)</label><label><input type="checkbox" checked={textOnly} disabled={imageSelected} onChange={(event) => setTextOnly(event.target.checked)} /> 원본 대신 텍스트만 학습 DB에 공개 {imageSelected && "(이미지 암기 영역을 선택해 사용할 수 없음)"}</label><label><input type="checkbox" checked={splitQuestionSet} onChange={(event) => setSplitQuestionSet(event.target.checked)} /> 번호가 있는 문제를 문제별로 나누기</label><label><input type="checkbox" checked={createRecall} onChange={(event) => setCreateRecall(event.target.checked)} /> 규칙 기반 능동 회상 카드 만들기</label></fieldset>
    <section className="materials-launcher"><div><span>직접 만든 학습 도구</span><strong>원문 선택 · 암기 · 회상 · 3D 개념도</strong><p>문장은 드래그해 고르고, 사진은 필요한 부분을 잘라 암기 자료로 만듭니다.</p></div><button className="secondary-button" type="button" disabled={!file} onClick={() => setMaterialsOpen(true)}>자료 구체화 {hasCustomMaterials(customMaterials) ? "수정" : "시작"}</button></section>
    <label><span>출처와 설명</span><textarea value={sourceNote} onChange={(event) => setSourceNote(event.target.value)} placeholder="자료의 출처와 어떤 학습에 도움이 되는지 적어주세요." rows={5} /></label><label className="check-row"><input type="checkbox" checked={licenseConfirmed} onChange={(event) => setLicenseConfirmed(event.target.checked)} /><span>이 자료를 공유할 권한이 있으며, {publishMode === "instant" ? "검수 없이 즉시 공개되고 크레딧이 지급되지 않는 것" : "AI가 추출 텍스트를 기준으로 검수하고 통과한 경우에만 공개·보상되는 것"}에 동의합니다.</span></label><button className="primary-button wide" type="submit" disabled={submitting || extracting || !file || !title || !licenseConfirmed}>{submitting ? "자료를 처리하고 있어요…" : publishMode === "instant" ? "즉시 공개하기 · 0 크레딧" : "AI 검수 요청하기 · 통과 시 +20"}</button>{result && <div className={result.ok ? "submission-result success" : "submission-result error"}><strong>{result.ok ? "접수 완료" : "확인 필요"}</strong><p>{result.message}</p>{result.status && <span>현재 상태 · {result.status}</span>}</div>}</form></div>{materialsOpen && <CustomMaterialsDialogV2 materials={customMaterials} file={file} extractedText={extractedText} onChange={setCustomMaterials} onClose={() => setMaterialsOpen(false)} />}</main>;
}

function CustomMaterialsDialogV2({ materials, file, extractedText, onChange, onClose }: { materials: CustomMaterials; file: File | null; extractedText: string; onChange: (materials: CustomMaterials) => void; onClose: () => void }) {
...
function ImageCropPicker({ file, onAdd }: { file: File; onAdd: (selection: SourceSelection) => void }) {
  const [src, setSrc] = useState(""); const [start, setStart] = useState<{ x: number; y: number } | null>(null); const [crop, setCrop] = useState<{ x: number; y: number; width: number; height: number } | null>(null); const image = useRef<HTMLImageElement>(null);
  ...
  return <section className="source-picker"><div className="authoring-title"><h3>사진에서 암기할 부분 고르기</h3><button className="text-button" type="button" disabled={!crop} onClick={() => { if (crop) { onAdd({ kind: "image", ...crop, label: "사진에서 선택한 암기 영역" }); setCrop(null); } }}>선택 영역 추가</button></div><div className="crop-picker">{src && <img ref={image} src={src} alt="암기할 영역을 드래그해 선택할 원본" onPointerDown={(event) => { event.currentTarget.setPointerCapture(event.pointerId); setStart(point(event)); setCrop(null); }} onPointerMove={(event) => { if (!start) return; const end = point(event); setCrop({ x: Math.min(start.x, end.x), y: Math.min(start.y, end.y), width: Math.abs(end.x - start.x), height: Math.abs(end.y - start.y) }); }} onPointerUp={() => setStart(null)} />}{crop && <span className="crop-overlay" style={{ left: `${crop.x * 100}%`, top: `${crop.y * 100}%`, width: `${crop.width * 100}%`, height: `${crop.height * 100}%` }} />}</div><small>사진을 드래그해 영역을 정한 뒤 추가하세요. 이 기능을 사용하면 원본도 공개됩니다.</small></section>;
}
...
function CustomMaterialsPanelV2({ materials, imageUrl }: { materials: CustomMaterials; imageUrl?: string }) {

...
function ContributionScreenV2({ onBack, onPublished }: { onBack: () => void; onPublished: (item: ContributionRecord) => void }) {
  ...
  async function chooseFiles(next: FileList | File[] | null, append = false) {
    ...
    if (!title.trim() && merged[0]) setTitle(merged[0].name.replace(/\.[^.]+$/, "").slice(0, 160));
...
type ContributionWorkspaceProps = Parameters<typeof ContributionWorkspaceLegacy>[0];
...
function ContributionWorkspace(props: ContributionWorkspaceProps) {
  ...
  if (!props.files.length) {
    return <main className="contribution-workspace contribution-workspace-final"><button className="back-button" type="button" onClick={props.onBack}>← 검색으로 돌아가기</button><section className="contribution-start-card"><p className="eyebrow">자료 기여</p><h1>학습할 원문을 올려 주세요.</h1><p>자료를 고르면 원문과 네 가지 제작 도구만 남는 작업 화면으로 바뀝니다.</p><div className="contribution-start-actions"><label className="primary-button"><input type="file" multiple accept=".pdf,.png,.jpg,.jpeg,.webp,.txt,.md,.docx,.pptx" onChange={(event) => void props.chooseFiles(event.target.files)} />파일 선택</label><label className="secondary-button"><input type="file" multiple accept="image/png,image/jpeg,image/webp" capture="environment" onChange={(event) => void props.chooseFiles(event.target.files, true)} />사진 촬영</label></div><small>PDF, Word, 이미지, 텍스트 · 최대 5개</small></section></main>;
  }

  return <main className="contribution-workspace contribution-workspace-final"><button className="back-button" type="button" onClick={props.onBack}>← 검색으로 돌아가기</button><form onSubmit={props.submit} className="final-contribution-form"><ContributionSourceCanvas files={props.files} extractedText={props.extractedText} loading={props.filesLoading} onAddFiles={props.chooseFiles} onAddSelection={addSelection} selectionCount={props.customMaterials.memorization.selections.length} /><section className="final-tool-column"><div className="contribution-tool-stack">{tools.map((tool) => <section className={openTool === tool.key ? "contribution-tool active" : "contribution-tool"} key={tool.key}><button className="contribution-tool-capsule" type="button" aria-expanded={openTool === tool.key} onClick={() => toggleTool(tool.key)}><span><strong>{tool.label}</strong><small>{tool.hint}</small></span><b>{openTool === tool.key ? "−" : "→"}</b></button>{openTool === tool.key && <div className="contribution-tool-drawer">{tool.key === "description" && <DescriptionContributionPane props={props} />}{tool.key === "memory" && <MemoryContributionPane materials={props.customMaterials} onChange={props.setCustomMaterials} />}{tool.key === "examples" && <ExamplesContributionPane materials={props.customMaterials} onChange={props.setCustomMaterials} />}{tool.key === "concept" && <ConceptCanvasEditor canvases={props.customMaterials.recall.conceptCanvases} onChange={(conceptCanvases) => setRecall({ ...props.customMaterials.recall, conceptCanvases })} />}</div>}</section>)}</div><footer className="final-publish-bar"><div className="final-publish-settings"><div className="publish-toggle"><button type="button" className={props.publishMode === "instant" ? "active" : ""} onClick={() => props.setPublishMode("instant")}>즉시 공개</button><button type="button" className={props.publishMode === "ai_review" ? "active" : ""} onClick={() => props.setPublishMode("ai_review")}>AI 검수 · +20 C</button></div><label className="check-row"><input type="checkbox" checked={props.licenseConfirmed} onChange={(event) => props.setLicenseConfirmed(event.target.checked)} /><span>공유 권한과 공개 방식에 동의합니다.</span></label></div><button className="primary-button final-publish-button" type="submit" disabled={props.submitting || props.filesLoading || !props.title || !props.licenseConfirmed}>{props.submitting ? "처리 중…" : "자료 공개하기"}</button>{props.result && <div className={props.result.ok ? "submission-result success" : "submission-result error"}><strong>{props.result.ok ? "접수 완료" : "확인 필요"}</strong><p>{props.result.message}</p></div>}</footer></section></form></main>;
}
...
function ContributionSourceCanvas({ files, extractedText, loading, onAddFiles, onAddSelection, selectionCount }: { files: File[]; extractedText: string; loading: boolean; onAddFiles: ContributionWorkspaceProps["chooseFiles"]; onAddSelection: (selection: SourceSelection) => void; selectionCount: number }) {
  ...
  useEffect(() => { if (!file) return; const url = URL.createObjectURL(file); setSrc(url); return () => URL.revokeObjectURL(url); }, [file]);

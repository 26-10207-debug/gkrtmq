  ...
  useEffect(() => {
    ...
    async function openLinkedMaterial() {
      if (kind === "contribution") {
        ...
        if (active) { setSelectedAsset(contributionToAsset(data.contributions[0])); setView("detail"); }
      } else if (kind === "folder") {
        const response = await fetch(`/api/folders?id=${encodeURIComponent(id)}`);
        const data = await response.json() as { folder?: FolderRecord };
        if (!response.ok || !data.folder) throw new Error("공개 폴더를 찾을 수 없습니다.");
        if (active) { setSelectedFolder(data.folder); setView("folder"); }
      } else if (kind === "reference") {
        const response = await fetch(`/api/public/materials/${encodeURIComponent(`${kind}:${id}`)}`);
        if (!response.ok) throw new Error("참고 자료를 찾을 수 없습니다.");
        const data = await response.json() as { title: string; text: string; metadata: { sourceUrl: string; sourceName: string; licenseNote: string } };
...
function Header(props: {
  ...
  return (
    <header className={props.isHome ? "topbar topbar-home" : "topbar"}>
      ...
      <div className="header-actions"><a className="contribute-button connection-nav" href="/connect">GPT 연결</a>
    ...
    <main className="contribution-main">
      ...
      <div className="contribution-grid">
        ...
        <form className="contribution-form" onSubmit={submit}>
          ...
          <label className="upload-field"><span>파일</span><input type="file" onChange={(event) => setFile(event.target.files?.[0] ?? null)} required /><div><strong>{file ? file.name : "파일을 선택하세요"}</strong><small>{file ? `${(file.size / 1024 / 1024).toFixed(2)}MB` : "PDF, 이미지, 문서 · 최대 20MB"}</small></div></label>
...
function ContributionScreenV2({ onBack, onPublished, initialDraft }: { onBack: () => void; onPublished: (item: ContributionRecord) => void; initialDraft?: DraftRecord | null }) {
  ...
  async function chooseFiles(next: FileList | File[] | null, append = false) {
    ...
    if (append && merged.length === files.length) return;
    if (merged.length > MAX_UPLOAD_COUNT || merged.some((file) => file.size > MAX_UPLOAD_BYTES) || merged.reduce((sum, file) => sum + file.size, 0) > MAX_TOTAL_UPLOAD_BYTES) {
      setResult({ ok: false, message: "파일당 20MB, 최대 5개, 합계 32MB까지 추가할 수 있습니다. 웹 자료의 여러 파일은 ZIP으로 묶어 주세요." }); return;
    }
    const candidateTitle = title.trim() || merged[0].name.replace(/\.[^.]+$/, "").slice(0, 160);
    uploadingRef.current = true; setExtracting(true); setDraftState("saving"); setResult(null);
    try {
      const readings = await Promise.all(merged.map(async (file) => {
        if (isImageFile(file)) return { text: "", message: "사진 · 필요하면 OCR로 본문을 읽을 수 있습니다." };
        try { const text = await extractDocumentText(file); return { text, message: text ? `본문·코드 ${text.length.toLocaleString("ko-KR")}자를 읽었습니다.` : "원본 파일로 보관합니다." }; }
        catch (error) { return { text: "", message: error instanceof Error ? error.message : "미리보기를 만들지 못해 원본으로 보관합니다." }; }
      }));
      const nextTexts = readings.map((reading, index) => append && index < files.length ? (extractedTexts[index] || reading.text) : reading.text);
      const body = new FormData(); (draftId && append ? merged.slice(files.length) : merged).forEach((file) => body.append("files", file));
      if (draftId) { body.set("id", draftId); if (!append) body.set("replace", "true"); }
      body.set("title", candidateTitle); body.set("subject", subject);
      ...
      setDraftState("saved");
    } catch (error) { setDraftState("error"); setResult({ ok: false, message: error instanceof Error ? error.message : "파일을 저장하지 못했습니다." }); }
    finally { uploadingRef.current = false; setExtracting(false); }
  }
  ...
  return <main className="contribution-main"><button className="back-button" type="button" onClick={onBack}>← 검색으로 돌아가기</button><div className="contribution-grid"><section className="contribution-intro"><p className="eyebrow">자료 기여</p><h1>원문에서 학습 도구까지 만드세요.</h1><p>문서의 글자를 바로 읽고, 암기·회상·예시 도구를 기여 자료와 함께 공개합니다.</p><ol><li><span>1</span><div><strong>원문 바로 읽기</strong><p>Word와 텍스트 PDF는 OCR 없이 본문을 추출합니다.</p></div></li><li><span>2</span><div><strong>자료 구체화</strong><p>원문에서 필요한 부분을 골라 암기와 회상 도구로 바꿉니다.</p></div></li><li><span>3</span><div><strong>공개·검수</strong><p>AI 검수에는 원본 대신 추출 텍스트만 전달됩니다.</p></div></li></ol></section><form className="contribution-form" onSubmit={submit}>
    ...
    <label className="upload-field"><span>파일 여러 개</span><input type="file" multiple onChange={(event) => void chooseFiles(event.target.files)} /><div><strong>{files.length ? `${files.length}개 파일 선택됨` : "파일을 선택하세요"}</strong><small>{files.length ? "사진과 문서를 한 자료로 묶어 공개합니다." : "모든 형식 · 파일당 20MB · 최대 5개 · 합계 32MB"}</small></div></label>
...
function ContributionWorkspace(props: ContributionWorkspaceProps) {
  ...
  const [toolsFocus, setToolsFocus] = useState(false);
  const [mobilePane, setMobilePane] = useState<"source" | "tools">("source");
  const addSelection = (selection: SourceSelection) => props.setCustomMaterials({ ...props.customMaterials, memorization: { ...props.customMaterials.memorization, selections: [...props.customMaterials.memorization.selections, selection] } });
  ...
  const fileLabel = props.files.length ? `${props.files[0].name}${props.files.length > 1 ? ` 외 ${props.files.length - 1}개` : ""}` : "파일 없음";
  return <main className={`contribution-workspace contribution-workspace-final mobile-pane-${mobilePane}`}><button className="back-button contribution-back" type="button" onClick={props.onBack}>← 검색으로</button>{props.result && !props.result.ok && <p className="upload-inline-error" role="alert">{props.result.message}</p>}<nav className="mobile-workspace-tabs" aria-label="업로드 작업 화면"><button type="button" aria-pressed={mobilePane === "source"} onClick={() => setMobilePane("source")}>자료</button><button type="button" aria-pressed={mobilePane === "tools"} onClick={() => setMobilePane("tools")}>편집 도구·공개</button></nav><form onSubmit={props.submit} className={`final-contribution-form ${toolsFocus ? "tools-focus" : ""}`}><ContributionSourceCanvas draftId={props.draftId} files={props.files} extractedText={props.extractedText} loading={props.filesLoading} onAddFiles={props.chooseFiles} onAddSelection={addSelection} selectionCount={props.customMaterials.memorization.selections.length} /><button className="workspace-split-toggle" type="button" aria-label={toolsFocus ? "원문 영역 넓게 보기" : "도구 영역 넓게 보기"} aria-pressed={toolsFocus} onClick={() => setToolsFocus((value) => !value)}>{toolsFocus ? "›" : "‹"}</button><section className="final-tool-column"><div className="workspace-compact-meta"><div className="workspace-meta-tags"><button type="button" onClick={() => setClassificationOpen((value) => !value)}>{props.subject || "분류 없음"} · 분류 수정</button><span title={fileLabel}>{fileLabel}</span><span>{folder ? `${folder.folderType === "book" ? "책" : "폴더"} · ${folder.title}` : "폴더 없음"}</span></div><div className={`workspace-save-actions draft-status ${props.draftState}`}><span>{statusLabel}</span><button className="secondary-button" type="button" onClick={props.saveDraft}>임시저장</button></div></div>{classificationOpen && <WorkspaceClassificationPanel props={props} onClose={() => setClassificationOpen(false)} />}<label className="workspace-title-field"><span>자료 제목</span><input value={props.title} onChange={(event) => props.setTitle(event.target.value)} placeholder="자료 제목" /></label><nav className="detail-menu-tabs" aria-label="자료 제작 메뉴">{([['description','설명'],['tools','도구'],['ai','AI']] as const).map(([key,label]) => <button key={key} type="button" className={menu === key ? "active" : ""} onClick={() => setMenu(key)}>{label}</button>)}</nav><div className="detail-menu-panel contribution-menu-panel">{menu === "description" && <DescriptionContributionPane props={props} />}{menu === "tools" && <ContributionToolsPanel props={props} selected={selectedTool} setSelected={setSelectedTool} />}{menu === "ai" && <AIContributionPane props={props} onSelectTool={(tool) => { setSelectedTool(tool); setMenu("tools"); }} />}</div><footer className="final-publish-bar"><div className="final-publish-settings"><div className="publish-toggle"><button type="button" disabled={props.aiReviewLocked} className={props.publishMode === "instant" ? "active" : ""} onClick={() => props.setPublishMode("instant")}>즉시 공개</button><button type="button" className={props.publishMode === "ai_review" ? "active" : ""} onClick={() => props.setPublishMode("ai_review")}>AI 검수 · +20 C</button></div>{props.aiReviewLocked && <p className="ai-lock-note">AI 변경안을 적용해 이 초안은 AI 검수 공개로 고정되었습니다.</p>}<label className="check-row"><input type="checkbox" checked={props.licenseConfirmed} onChange={(event) => props.setLicenseConfirmed(event.target.checked)} /><span>공유 권한과 공개 방식에 동의합니다.</span></label>{!props.subject.trim() && <p className="classification-required-note">공개 전에 ‘분류 수정’에서 과목·분류를 정해 주세요.</p>}</div><button className="primary-button final-publish-button" type="submit" disabled={props.submitting || props.filesLoading || !props.files.length || !props.title || !props.subject.trim() || !props.licenseConfirmed}>{props.submitting ? "처리 중…" : props.revision ? "업데이트 공개" : "자료 공개하기"}</button>{props.result && <div className={props.result.ok ? "submission-result success" : "submission-result error"}><strong>{props.result.ok ? "접수 완료" : "확인 필요"}</strong><p>{props.result.message}</p></div>}</footer></section></form></main>;
}

function WorkspaceClassificationPanel({ props, onClose }: { props: ContributionWorkspaceProps; onClose: () => void }) {
  const regularFolders = props.folders.filter((item) => item.folderType !== "book");
  const bookFolders = props.folders.filter((item) => item.folderType === "book");
  const selectedBook = bookFolders.find((item) => item.id === props.folderId);
  return <section className="workspace-inline-classification" aria-label="분류 수정"><header><div><strong>분류·태그·폴더</strong><small>입력과 동시에 초안에 자동저장됩니다.</small></div><button type="button" onClick={onClose}>완료</button></header><div className="workspace-inline-fields"><label><span>과목·분류 <b>필수</b></span><input list="workspace-subjects" value={props.subject} onChange={(event) => props.setSubject(event.target.value)} placeholder="선택하거나 직접 입력" /><datalist id="workspace-subjects"><option value="물리학"/><option value="영어"/><option value="수학"/><option value="철학"/><option value="역사"/><option value="기타"/></datalist></label><label><span>태그 <small>쉼표로 구분</small></span><input value={props.tags} onChange={(event) => props.setTags(event.target.value)} placeholder="예: 가족 유사성, 언어게임" /></label><fieldset><legend>일반 폴더 <small>여러 개 선택 가능</small></legend>{regularFolders.length ? <div>{regularFolders.map((item) => <label key={item.id}><input type="checkbox" checked={props.regularFolderIds.includes(item.id)} onChange={() => props.setRegularFolderIds((current) => current.includes(item.id) ? current.filter((id) => id !== item.id) : [...current, item.id])} />{item.title}</label>)}</div> : <small>홈의 ‘폴더 만들기’에서 먼저 만들 수 있습니다.</small>}</fieldset><label><span>책 폴더 <small>하나만 선택</small></span><select value={props.folderId} onChange={(event) => { props.setFolderId(event.target.value); props.setPageStart(""); props.setPageEnd(""); }}><option value="">책 폴더 없음</option>{bookFolders.map((item) => <option key={item.id} value={item.id}>{item.title}</option>)}</select></label>{selectedBook && <div className="workspace-page-fields"><label><span>시작 페이지</span><input type="number" min="1" value={props.pageStart} onChange={(event) => props.setPageStart(event.target.value ? Number(event.target.value) : "")} /></label><label><span>끝 페이지 <small>한 페이지면 생략</small></span><input type="number" min={Number(props.pageStart) || 1} value={props.pageEnd} onChange={(event) => props.setPageEnd(event.target.value ? Number(event.target.value) : "")} /></label></div>}</div></section>;
}
...
function ContributionSourceCanvas({ draftId, files, extractedText, loading, onAddFiles, onAddSelection, selectionCount }: { draftId?: string; files: File[]; extractedText: string; loading: boolean; onAddFiles: ContributionWorkspaceProps["chooseFiles"]; onAddSelection: (selection: SourceSelection) => void; selectionCount: number }) {
  ...
  const finishCrop = (event: React.PointerEvent<HTMLImageElement>) => { if (!file || !start || mode !== "crop") return; const end = point(event); const next = { x: Math.min(start.x, end.x), y: Math.min(start.y, end.y), width: Math.abs(end.x - start.x), height: Math.abs(end.y - start.y) }; setStart(null); if (next.width > .02 && next.height > .02) { setCrop(next); onAddSelection({ kind: "image", ...next, attachmentIndex: activeIndex, label: `${file.name}에서 선택한 암기 영역` }); } };
  ...
  return <section className="final-source-column"><div className="final-source-stage"><aside className="final-source-tools"><label title="원문 추가">＋<input type="file" multiple onChange={(event) => void onAddFiles(event.target.files, true)} /><small>추가</small></label><label title="사진 촬영">◉<input type="file" multiple accept="image/png,image/jpeg,image/webp" capture="environment" onChange={(event) => void onAddFiles(event.target.files, true)} /><small>촬영</small></label><button type="button" disabled={!file} onClick={() => setZoom((value) => Math.min(3, value + .15))}>＋<small>확대</small></button><button type="button" disabled={!file} onClick={() => setZoom((value) => Math.max(.55, value - .15))}>−<small>축소</small></button><button className={mode === "move" ? "active" : ""} type="button" disabled={!file} onClick={() => setMode("move")}>✥<small>이동</small></button><button className={mode === "crop" ? "active" : ""} type="button" disabled={!file || !isImageFile(file)} onClick={() => setMode("crop")}>⌗<small>자르기</small></button><button type="button" disabled={!file} onClick={resetView}>↺<small>초기화</small></button></aside><div className="source-stage-status"><div className="source-file-nav"><button type="button" disabled={activeIndex === 0} onClick={() => setActiveIndex((value) => Math.max(0, value - 1))}>←</button><span>{file?.name || "파일 업로드"}</span><button type="button" disabled={!file || activeIndex >= files.length - 1} onClick={() => setActiveIndex((value) => Math.min(files.length - 1, value + 1))}>→</button></div><small>{files.length > 1 ? `${activeIndex + 1} / ${files.length}` : file ? loading ? "원문을 읽는 중" : "원문 준비 완료" : "최대 5개"}</small></div><div ref={viewportRef} className={`source-stage-viewport ${mode}`} style={{ "--source-zoom": zoom } as CSSProperties} onDoubleClick={resetView} onPointerDown={file && isImageFile(file) ? pointerDown : undefined} onPointerMove={pointerMove} onPointerUp={pointerUp} onPointerCancel={pointerUp}>{file && isWebPackage(file.name) ? <MaterialPlayer sourceId={draftId} kind="draft" attachment={activeIndex} /> : file && file.type.startsWith("video/") && src ? <video src={src} controls playsInline preload="metadata" onPointerDown={(event) => event.stopPropagation()} /> : file && file.type.startsWith("audio/") && src ? <audio src={src} controls preload="metadata" onPointerDown={(event) => event.stopPropagation()} /> : file && isTextSource(file.name, file.type) ? <SourceTextPreview file={file} /> : file && isImageFile(file) && src ? <div className="source-image-wrap"><img src={src} alt={file.name} draggable={false} onPointerDown={(event) => { if (mode !== "crop") return; event.stopPropagation(); event.currentTarget.setPointerCapture(event.pointerId); setStart(point(event)); setCrop(null); }} onPointerMove={(event) => { if (!start || mode !== "crop") return; const end = point(event); setCrop({ x: Math.min(start.x, end.x), y: Math.min(start.y, end.y), width: Math.abs(end.x - start.x), height: Math.abs(end.y - start.y) }); }} onPointerUp={finishCrop} />{crop && <span className="crop-overlay" style={{ left: `${crop.x * 100}%`, top: `${crop.y * 100}%`, width: `${crop.width * 100}%`, height: `${crop.height * 100}%` }} />}</div> : file && /\.pdf$/i.test(file.name) && src ? <iframe src={src} title={`${file.name} 미리보기`} /> : extractedText ? <textarea readOnly value={extractedText} onSelect={(event) => { const input = event.currentTarget; setSelectedText(input.value.slice(input.selectionStart, input.selectionEnd).trim()); }} aria-label="원문 텍스트" /> : file ? <div className="source-document-placeholder"><b>{file.name.split(".").pop()?.toUpperCase() || "FILE"}</b><strong>{file.name}</strong><span>{loading ? "본문을 읽고 있어요…" : "원본으로 보관됩니다. 이 형식은 다운로드해서 열어 주세요."}</span></div> : <div className="source-upload-placeholder"><b>▧</b><strong>파일 업로드</strong><span>문서·이미지·영상·코드·웹 게임 ZIP을 추가하세요.<br />파일당 20MB · 최대 5개 · 합계 32MB</span><div><label className="primary-button">파일 선택<input type="file" multiple onChange={(event) => void onAddFiles(event.target.files, true)} /></label><label className="secondary-button">사진 촬영<input type="file" multiple accept="image/png,image/jpeg,image/webp" capture="environment" onChange={(event) => void onAddFiles(event.target.files, true)} /></label></div></div>}</div>{selectedText && <button className="source-selection-action" type="button" onClick={() => { onAddSelection({ kind: "text", value: selectedText }); setSelectedText(""); }}>선택 문장을 암기 자료로 추가</button>}{selectionCount > 0 && <span className="source-selection-count">선택 영역 {selectionCount}</span>}</div></section>;
}
...
function ImageFixAuthoring({ props }: { props: ContributionWorkspaceProps }) {
  ...
  const applyReadingMode = (mode: "color" | "soft" | "gray" | "contrast") => {
    const settings = mode === "color" ? { brightness: 100, contrast: 108, grayscale: false } : mode === "soft" ? { brightness: 108, contrast: 116, grayscale: false } : mode === "gray" ? { brightness: 110, contrast: 126, grayscale: true } : { brightness: 118, contrast: 155, grayscale: true };
    save({ ...fix, ...settings });
  };
  const copySettingsToAll = () => {
    const imageSet = new Set(imageIndices);
    const untouched = props.customMaterials.tools.imageFixes.filter((item) => !imageSet.has(item.attachmentIndex));
    const copied = imageIndices.map((attachmentIndex) => { const current = props.customMaterials.tools.imageFixes.find((item) => item.attachmentIndex === attachmentIndex); return { ...fix, attachmentIndex, corners: current?.corners || [{x:.05,y:.05},{x:.95,y:.05},{x:.95,y:.95},{x:.05,y:.95}], correctedAttachmentIndex: current?.correctedAttachmentIndex }; });
    props.setCustomMaterials({ ...props.customMaterials, tools: { ...props.customMaterials.tools, imageFixes: [...untouched, ...copied] } });
    setMessage(`${imageIndices.length}개 이미지에 읽기 설정을 적용했습니다. 경계는 이미지마다 따로 유지됩니다.`);
  };
  ...
  return <div className="image-fix-authoring"><label><span>읽기 편하게 만들 이미지</span><select value={index} onChange={(event) => setIndex(Number(event.target.value))}>{imageIndices.map((item) => <option key={item} value={item}>{props.files[item].name}</option>)}</select></label><div className="image-fix-presets" aria-label="문서 읽기 모드"><button type="button" onClick={() => applyReadingMode("color")}>원본 컬러</button><button type="button" onClick={() => applyReadingMode("soft")}>밝은 문서</button><button type="button" onClick={() => applyReadingMode("gray")}>회색 문서</button><button type="button" onClick={() => applyReadingMode("contrast")}>고대비 흑백</button></div><div className="image-fix-live corner-preview"><img src={src} alt="보정 미리보기" style={{ transform: `rotate(${fix.rotation}deg)`, filter: `brightness(${fix.brightness}%) contrast(${fix.contrast}%) grayscale(${fix.grayscale ? 1 : 0})` }} />{fix.corners.map((corner,cornerIndex)=><span key={cornerIndex} style={{left:`${corner.x*100}%`,top:`${corner.y*100}%`}}>{cornerIndex+1}</span>)}</div><div className="image-fix-actions"><button className="secondary-button" type="button" disabled={busy} onClick={()=>void detect()}>자동 경계 감지</button><button className="primary-button" type="button" disabled={busy} onClick={()=>void store()}>{busy?"처리 중…":"읽기용 WebP 저장"}</button></div><div className="corner-controls">{fix.corners.map((corner,cornerIndex)=><fieldset key={cornerIndex}><legend>모서리 {cornerIndex+1}</legend><label><span>X {Math.round(corner.x*100)}%</span><input type="range" min="0" max="100" value={corner.x*100} onChange={(event)=>save({...fix,corners:fix.corners.map((point,itemIndex)=>itemIndex===cornerIndex?{...point,x:Number(event.target.value)/100}:point)})}/></label><label><span>Y {Math.round(corner.y*100)}%</span><input type="range" min="0" max="100" value={corner.y*100} onChange={(event)=>save({...fix,corners:fix.corners.map((point,itemIndex)=>itemIndex===cornerIndex?{...point,y:Number(event.target.value)/100}:point)})}/></label></fieldset>)}</div><div className="image-fix-controls"><label><span>회전 {fix.rotation}°</span><input type="range" min="-15" max="15" value={fix.rotation} onChange={(event) => save({ ...fix, rotation: Number(event.target.value) })} /></label><label><span>밝기 {fix.brightness}%</span><input type="range" min="60" max="160" value={fix.brightness} onChange={(event) => save({ ...fix, brightness: Number(event.target.value) })} /></label><label><span>대비 {fix.contrast}%</span><input type="range" min="60" max="160" value={fix.contrast} onChange={(event) => save({ ...fix, contrast: Number(event.target.value) })} /></label><label className="check-row"><input type="checkbox" checked={fix.grayscale} onChange={(event) => save({ ...fix, grayscale: event.target.checked })} /><span>흑백 문서 모드</span></label></div><div className="image-fix-bottom-actions"><button className="secondary-button" type="button" onClick={() => save({ ...fix, rotation: 0, brightness: 100, contrast: 100, grayscale: false, corners:[{x:.05,y:.05},{x:.95,y:.05},{x:.95,y:.95},{x:.05,y:.95}] })}>보정 초기화</button>{imageIndices.length > 1 && <button className="secondary-button" type="button" onClick={copySettingsToAll}>이 읽기 설정을 모든 이미지에 적용</button>}</div>{message&&<p className="record-message">{message}</p>}<small>이미지별 경계는 따로 조정할 수 있고, 밝기·대비·흑백 설정만 한 번에 복사할 수 있습니다. 모든 처리는 브라우저 안에서 이루어집니다.</small></div>;
}
...
function QuestionSplitAuthoring({ props }: { props: ContributionWorkspaceProps }) {
  ...
  const replaceQuestions = (setIndex: number, questions: CustomMaterials["tools"]["questionSets"][number]["questions"]) => save(sets.map((item,index) => index === setIndex ? { ...item, questions } : item));
  const move = (setIndex: number, questionIndex: number, delta: number) => { const questions=[...sets[setIndex].questions]; const target=questionIndex+delta; if(target<0||target>=questions.length)return; [questions[questionIndex],questions[target]]=[questions[target],questions[questionIndex]]; replaceQuestions(setIndex,questions); };
  const mergeNext = (setIndex: number, questionIndex: number) => { const questions=[...sets[setIndex].questions]; if(!questions[questionIndex+1])return; questions[questionIndex]={prompt:`${questions[questionIndex].prompt}\n\n${questions[questionIndex+1].prompt}`.trim(),answer:[questions[questionIndex].answer,questions[questionIndex+1].answer].filter(Boolean).join("\n")};questions.splice(questionIndex+1,1);replaceQuestions(setIndex,questions); };
  const splitParagraphs = (setIndex: number, questionIndex: number) => { const questions=[...sets[setIndex].questions]; const originalAnswer=questions[questionIndex]?.answer||""; const parts=questions[questionIndex].prompt.split(/\n\s*\n/).map((part)=>part.trim()).filter(Boolean);if(parts.length<2)return;questions.splice(questionIndex,1,...parts.map((prompt,index)=>({prompt,answer:index===0?originalAnswer:""})));replaceQuestions(setIndex,questions); };
  return <div className="question-authoring"><div className="authoring-title"><div><h3>번호 규칙으로 문항 분리</h3><p>AI 호출 없이 분리하고 직접 수정·병합·재정렬할 수 있습니다.</p></div><button className="secondary-button" type="button" disabled={!props.extractedText.trim()} onClick={generate}>원문에서 분리</button></div>{!sets.length && <button className="empty-authoring-button" type="button" onClick={() => save([{ title: "새 문항 세트", questions: [{ prompt: "", answer: "" }] }])}>빈 문항 세트 만들기</button>}{sets.map((set, setIndex) => <section className="question-set-editor" key={setIndex}><input value={set.title} aria-label="문항 세트 제목" onChange={(event) => save(sets.map((item,index) => index === setIndex ? { ...item, title: event.target.value } : item))} />{set.questions.map((question, questionIndex) => <article key={questionIndex}><div className="question-order-controls"><span>{questionIndex + 1}</span><button type="button" disabled={questionIndex===0} title="위로" onClick={()=>move(setIndex,questionIndex,-1)}>↑</button><button type="button" disabled={questionIndex===set.questions.length-1} title="아래로" onClick={()=>move(setIndex,questionIndex,1)}>↓</button></div><textarea rows={3} value={question.prompt} placeholder="문항" onChange={(event) => replaceQuestions(setIndex,set.questions.map((entry,qIndex)=>qIndex===questionIndex?{...entry,prompt:event.target.value}:entry))} /><textarea rows={2} value={question.answer} placeholder="정답·해설 (선택)" onChange={(event) => replaceQuestions(setIndex,set.questions.map((entry,qIndex)=>qIndex===questionIndex?{...entry,answer:event.target.value}:entry))} /><div className="question-row-actions"><button type="button" disabled={questionIndex===set.questions.length-1} onClick={()=>mergeNext(setIndex,questionIndex)}>다음과 병합</button><button type="button" onClick={()=>splitParagraphs(setIndex,questionIndex)}>빈 줄로 분할</button><button type="button" className="remove-question" aria-label="문항 삭제" onClick={() => replaceQuestions(setIndex,set.questions.filter((_,qIndex)=>qIndex!==questionIndex))}>삭제</button></div></article>)}<button className="text-button" type="button" onClick={() => replaceQuestions(setIndex,[...set.questions,{prompt:"",answer:""}])}>+ 문항 추가</button></section>)}</div>;
}

  ...
  useEffect(() => {
    ...
    async function openLinkedMaterial() {
      if (kind === "contribution") {
        ...
        if (!data.folders?.[0]) throw new Error("공개 폴더를 찾을 수 없습니다.");
        if (active) { setSelectedFolder(data.folders[0]); setView("folder"); }
    ...
    <main className="contribution-main">
      ...
      <div className="contribution-grid">
        ...
        <form className="contribution-form" onSubmit={submit}>
          ...
          <label className="upload-field"><span>파일</span><input type="file" onChange={(event) => setFile(event.target.files?.[0] ?? null)} required /><div><strong>{file ? file.name : "파일을 선택하세요"}</strong><small>{file ? `${(file.size / 1024 / 1024).toFixed(2)}MB` : "PDF, 이미지, 문서 · 최대 8MB"}</small></div></label>
...
function ContributionScreenV2({ onBack, onPublished, initialDraft }: { onBack: () => void; onPublished: (item: ContributionRecord) => void; initialDraft?: DraftRecord | null }) {
  ...
  async function chooseFiles(next: FileList | File[] | null, append = false) {
    ...
    if (merged.length > MAX_UPLOAD_COUNT || merged.some((file) => file.size > MAX_UPLOAD_BYTES) || merged.reduce((sum, file) => sum + file.size, 0) > MAX_TOTAL_UPLOAD_BYTES) {
      setResult({ ok: false, message: "파일당 8MB, 최대 5개, 합계 32MB까지 추가할 수 있습니다. 웹 자료의 여러 파일은 ZIP으로 묶어 주세요." }); return;
    }
  ...
  return <main className="contribution-main"><button className="back-button" type="button" onClick={onBack}>← 검색으로 돌아가기</button><div className="contribution-grid"><section className="contribution-intro"><p className="eyebrow">자료 기여</p><h1>원문에서 학습 도구까지 만드세요.</h1><p>문서의 글자를 바로 읽고, 암기·회상·예시 도구를 기여 자료와 함께 공개합니다.</p><ol><li><span>1</span><div><strong>원문 바로 읽기</strong><p>Word와 텍스트 PDF는 OCR 없이 본문을 추출합니다.</p></div></li><li><span>2</span><div><strong>자료 구체화</strong><p>원문에서 필요한 부분을 골라 암기와 회상 도구로 바꿉니다.</p></div></li><li><span>3</span><div><strong>공개·검수</strong><p>AI 검수에는 원본 대신 추출 텍스트만 전달됩니다.</p></div></li></ol></section><form className="contribution-form" onSubmit={submit}>
    ...
    <label className="upload-field"><span>파일 여러 개</span><input type="file" multiple onChange={(event) => void chooseFiles(event.target.files)} /><div><strong>{files.length ? `${files.length}개 파일 선택됨` : "파일을 선택하세요"}</strong><small>{files.length ? "사진과 문서를 한 자료로 묶어 공개합니다." : "모든 형식 · 파일당 8MB · 최대 5개 · 합계 32MB"}</small></div></label>

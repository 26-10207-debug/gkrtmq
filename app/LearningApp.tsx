...
function ContributionScreenV2({ onBack, onPublished, initialDraft }: { onBack: () => void; onPublished: (item: ContributionRecord) => void; initialDraft?: DraftRecord | null }) {
  ...
  const [phase, setPhase] = useState<"classification" | "workspace">("classification"); const [pageStart, setPageStart] = useState<number | "">(""); const [pageEnd, setPageEnd] = useState<number | "">("");
  ...
  useEffect(() => {
    ...
    setFolderId(initialDraft.folderId || ""); setRegularFolderIds(initialDraft.regularFolderIds || []); setPageStart(initialDraft.pageStart || ""); setPageEnd(initialDraft.pageEnd || ""); setPublishMode(initialDraft.publishMode || "instant"); setExtractedTexts(initialDraft.extractedTexts || []);
    const base = emptyCustomMaterials(); const incoming = initialDraft.customMaterials || {};
    setCustomMaterials({ ...base, ...incoming, memorization: { ...base.memorization, ...incoming.memorization }, recall: { ...base.recall, ...incoming.recall }, examples: incoming.examples || [] });
  ...
  if (phase === "classification") return <ContributionClassification onBack={onBack} subject={subject} setSubject={setSubject} tags={tags} setTags={setTags} folders={folders} setFolders={setFolders} folderId={folderId} setFolderId={setFolderId} regularFolderIds={regularFolderIds} setRegularFolderIds={setRegularFolderIds} pageStart={pageStart} setPageStart={setPageStart} pageEnd={pageEnd} setPageEnd={setPageEnd} onContinue={() => setPhase("workspace")} />;
...
function ContributionClassification({ onBack, subject, setSubject, tags, setTags, folders, setFolders, folderId, setFolderId, regularFolderIds, setRegularFolderIds, pageStart, setPageStart, pageEnd, setPageEnd, onContinue }: { onBack: () => void; subject: string; setSubject: (value: string) => void; tags: string; setTags: (value: string) => void; folders: FolderRecord[]; setFolders: (value: FolderRecord[] | ((current: FolderRecord[]) => FolderRecord[])) => void; folderId: string; setFolderId: (value: string) => void; regularFolderIds: string[]; setRegularFolderIds: (value: string[] | ((current: string[]) => string[])) => void; pageStart: number | ""; setPageStart: (value: number | "") => void; pageEnd: number | ""; setPageEnd: (value: number | "") => void; onContinue: () => void }) {
  const [creating, setCreating] = useState(false); const [folderTitle, setFolderTitle] = useState(""); const [folderType, setFolderType] = useState<"regular" | "book">("regular"); const [message, setMessage] = useState("");
  const selectedFolder = folders.find((folder) => folder.id === folderId); const pagesValid = selectedFolder?.folderType !== "book" || (Number(pageStart) >= 1 && (!pageEnd || Number(pageEnd) >= Number(pageStart)));
  async function createFolder() { if (!folderTitle.trim()) return; setMessage(""); const response = await fetch("/api/folders", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ title: folderTitle, subject, tags, folderType }) }); const data = await response.json() as { folder?: FolderRecord; error?: string }; if (!response.ok || !data.folder) { setMessage(data.error || "폴더를 만들지 못했습니다."); return; } setFolders((current) => [data.folder!, ...current]); if (folderType === "book") setFolderId(data.folder.id); else setRegularFolderIds((current) => [...new Set([...current, data.folder!.id])]); setCreating(false); setFolderTitle(""); }
  const regularFolders = folders.filter((folder) => folder.folderType !== "book"); const bookFolders = folders.filter((folder) => folder.folderType === "book");
  return <main className="contribution-workspace classification-main"><button className="back-button" type="button" onClick={onBack}>← 검색으로 돌아가기</button><section className="classification-card"><div><p className="eyebrow">자료 기여 · 1단계</p><h1>자료의 자리를 먼저 정하세요.</h1><p>분류는 검색 기준이 되고, 일반 폴더와 페이지 기반 책 폴더를 함께 사용할 수 있습니다.</p></div><div className="classification-fields"><label><span>과목·분류 <b>필수</b></span><input list="classification-subjects" value={subject} onChange={(event) => setSubject(event.target.value)} placeholder="선택하거나 직접 입력" /><datalist id="classification-subjects"><option value="물리학"/><option value="영어"/><option value="수학"/><option value="철학"/><option value="역사"/><option value="기타"/></datalist></label><label><span>태그 <small>선택</small></span><input value={tags} onChange={(event) => setTags(event.target.value)} placeholder="쉼표로 구분" /></label><section className="folder-classification-group"><span>일반 폴더 <small>여러 개 선택 가능</small></span>{regularFolders.length ? <div>{regularFolders.map((folder) => <label key={folder.id}><input type="checkbox" checked={regularFolderIds.includes(folder.id)} onChange={() => setRegularFolderIds((current) => current.includes(folder.id) ? current.filter((id) => id !== folder.id) : [...current, folder.id])} />{folder.title}</label>)}</div> : <small>아직 일반 폴더가 없습니다.</small>}</section><label><span>책 폴더 <small>하나만 선택</small></span><div className="folder-select-row"><select value={folderId} onChange={(event) => { setFolderId(event.target.value); setPageStart(""); setPageEnd(""); }}><option value="">책 폴더 없이 공개</option>{bookFolders.map((folder) => <option key={folder.id} value={folder.id}>책 · {folder.title}</option>)}</select><button className="secondary-button" type="button" onClick={() => setCreating((value) => !value)}>새 폴더</button></div></label>{selectedFolder?.folderType === "book" && <div className="book-page-fields"><label><span>시작 페이지</span><input type="number" min="1" value={pageStart} onChange={(event) => setPageStart(event.target.value ? Number(event.target.value) : "")} /></label><label><span>끝 페이지 <small>한 페이지면 생략</small></span><input type="number" min={Number(pageStart) || 1} value={pageEnd} onChange={(event) => setPageEnd(event.target.value ? Number(event.target.value) : "")} /></label><p>표시 제목 · {selectedFolder.title} · {pageEnd && pageEnd !== pageStart ? `pp.${pageStart || 1}–${pageEnd}` : `p.${pageStart || 1}`}</p></div>}{creating && <section className="new-folder-inline"><div className="folder-type-choice"><button type="button" className={folderType === "regular" ? "active" : ""} onClick={() => setFolderType("regular")}>일반 폴더</button><button type="button" className={folderType === "book" ? "active" : ""} onClick={() => setFolderType("book")}>페이지 책 폴더</button></div><input value={folderTitle} onChange={(event) => setFolderTitle(event.target.value)} placeholder={folderType === "book" ? "예: 일반물리학 필기" : "예: 중간고사 준비"} /><button className="primary-button" type="button" onClick={() => void createFolder()}>폴더 만들기</button>{message && <p>{message}</p>}</section>}</div><button className="primary-button classification-continue" type="button" disabled={!subject.trim() || !pagesValid} onClick={onContinue}>원문과 학습 도구 만들기 →</button></section></main>;
}
...
type ContributionWorkspaceProps = Parameters<typeof ContributionWorkspaceLegacy>[0] & { onEditClassification: () => void; draftState: "idle" | "saving" | "saved" | "error"; saveDraft: () => void; revision: boolean };

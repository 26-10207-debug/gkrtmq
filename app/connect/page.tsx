...
export default function ConnectionGuide() {
  ...
  async function copy() { try { await navigator.clipboard.writeText(`${origin}/mcp`); setCopied(true); } catch { setCopied(false); } }
  return <main className="connection-guide">
    ...
    <section><h2>MCP 연결</h2><label htmlFor="mcp-url">연결 URL</label><div className="connection-url"><input id="mcp-url" readOnly value={origin ? `${origin}/mcp` : ""} onFocus={(event) => event.currentTarget.select()} /><button type="button" className="primary-button" onClick={copy}>{copied ? "복사됨" : "복사"}</button></div>
